...
public class CustomViewAbove extends ViewGroup {
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_DOWN:
			...
			// 增加侧边栏打开后，回到Above的灵敏度，只要鼠标按下，就开始跟着滑动
			// 改动没有全面测试，如果后续使用有问题，bug可能出在这里！！！
			if (isMenuOpen()) {
				startDrag();
//				mIsBeingDragged = true;
		...
		case MotionEvent.ACTION_UP:
			// 使用鼠标按下和抬起时间间隔来判断是否是一次点击
			long edgeTime = System.currentTimeMillis() - mCurrentTime;
			if (isMenuOpen() && edgeTime < 200) {
				setCurrentItem(1);
				endDrag();
				break;
			}
